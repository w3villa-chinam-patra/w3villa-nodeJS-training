...
function TypeAndSendSection({ setMessages }) {
  ...
  const sendHandler = () => {
    ...
    socket.emit("message-to-backend", message);
    setMessage("");
  };
  ...
  return (
    <div className="flex-none flex bg-gray-200 p-2 m-2 rounded-full gap-2 border border-gray-400">
      ...
      <button
        onClick={sendHandler}
        className="send-message-button px-4 py-2 flex-none cursor-pointer bg-fuchsia-600 border border-fuchsia-800 rounded-full text-white"
      >
        Send
      </button>
    </div>
